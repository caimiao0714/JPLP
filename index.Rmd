...
description: | 
  Modeling Recurrent Safety-critical Events among Commercial Truck Drivers:
  A Bayesian Hierarchical Jump Power Law Process
date: July 1, 2020
output: 
...
fit_NHPP = stan("Stan/nhpp_plp_hierarchical.stan",
                chains = 4, iter = 1000, warmup = 500, data = df_NHPP$hier_dat)
```
...
```{r}
fit_JPLP = stan("Stan/jplp_hierarchical_kappa02.stan",
                chains = 4, iter = 1000, warmup = 500, data = df_JPLP$stan_dt)
```
...
```{r}
t_NHPP = stan_trace(fit_NHPP, pars = 'mu0|sigma0|beta')
t_NHPP
```
...
```{r}
t_JPLP = stan_trace(fit_JPLP, pars = 'mu0|sigma0|beta|kappa')
t_JPLP
```
